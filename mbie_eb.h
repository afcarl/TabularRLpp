#ifndef TABULARRL_MBIE_EB_H
#define TABULARRL_MBIE_EB_H
...
class MBIE_EB {
public:
    MBIE_EB(int s, int a, int h, double beta, std::shared_ptr<TabularEnv> env);
    double execute();
...
MBIE_EB::MBIE_EB(int s, int a, int h, double beta, std::shared_ptr<TabularEnv> env):
s_(s), a_(a), h_(h), beta_(beta), env_(env),
...
double MBIE_EB::confidence_bounds(int s, int a, int t) {
    if (n_[nid(s, a, t)] > 0.0) {
        return beta_ / std::sqrt(n_[nid(s, a, t)]);
    } else {
        return inf;
    }
}

void MBIE_EB::update_policy() {
...
void MBIE_EB::train(int num_episodes, std::vector<double> &rewards) {
    double alpha = 0.005;
    double cul_reward = 0.0;
