#ifndef UBEV_EB_INFINITE_H
#define UBEV_EB_INFINITE_H
...
class UBEV_EB_Infinite {
public:
    UBEV_EB_Infinite(int s, int a, int h, double delta, double beta, double gamma, std::shared_ptr<TabularEnv> env);
    double execute();
...
UBEV_EB_Infinite::UBEV_EB_Infinite(int s, int a, int h, double delta, double beta, double gamma, std::shared_ptr<TabularEnv> env):
        s_(s), a_(a), h_(h), delta_(delta), beta_(beta), gamma_(gamma), env_(env),
        q_((unsigned)(s * a), 0), p_((unsigned)(s * a), 0), pi_((unsigned)(s), 0),
        n_((unsigned)(s * a), 0), m_((unsigned)(s * s * a), 0), l_((unsigned)(s * a), 0) {

}

double UBEV_EB_Infinite::execute() {
    int s = env_->state();
...
double UBEV_EB_Infinite::confidence_bounds(int s, int a) {
    if (n_[nid(s, a)] > 0.0) {
        return beta_ * std::sqrt(2 * llnp(nid(s, a)) + std::log(18 * s_ * a_ * 1.0 / (1 - gamma_) / delta_))
               / std::sqrt(n_[nid(s, a)]);
    } else {
        return inf;
    }
...
void UBEV_EB_Infinite::update_policy() {
...
void UBEV_EB_Infinite::train(int num_episodes, std::vector<double> &rewards) {
    double alpha = 0.005;
    double cul_reward = 0.0;
...
#endif //UBEV_EB_UBEV_EB_INFINITE_H
